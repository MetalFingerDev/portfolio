...
export default class Sun extends BaseBody implements ICelestialBody {
  ...
  constructor(_parent: THREE.Group | undefined, config: Configuration) {
    ...
    this.initGroups();
    this.setDetail(config.detailed ?? true);
  }

  protected initGroups(): void {
    const detailed = this.config.detailed ?? true;
    const radius = this.config.radius ?? 20;
    ...
    if (detailed) {
      const intensity = this.config.intensity ?? radius * 20;
